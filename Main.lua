-- V3.2 HUB MM2 - SINTAXIS CORREGIDA
...
local function Notify(title, text, color)
    ...
    f.Size = UDim2.new(0, 290, 0, 90)
    f.Position = UDim2.new(1, 20, 0.15, 0)
    f.BackgroundColor3 = Config.Colors.Bg
    Instance.new("UICorner", f)
    local s = Instance.new("UIStroke", f); s.Color = color; s.Thickness = 3
    ...
    task.delay(4, function()
        if f then
            f:TweenPosition(UDim2.new(1, 20, 0.15, 0), "In", "Quad", 0.6)
            task.wait(0.7)
            sg:Destroy()
        end
    end)
...
local function CreateESP(p)
    ...
    conn = RunService.RenderStepped:Connect(function()
        ...
        highlight.Enabled = Config.Toggles["ESP_" .. role] or false
...
for _, p in pairs(Players:GetPlayers()) do
    if p \~= lp then CreateESP(p) end   -- Corregido
end
...
local function InitMotors()
    RunService.Stepped:Connect(function()
        if Config.Toggles.KillAura and GetRole(lp) == "Murd" then
            ...
            if k then
                for _, p in pairs(Players:GetPlayers()) do
                    if p \~= lp and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then   -- Corregido
    ...
    RunService.RenderStepped:Connect(function()
        for _, p in pairs(Players:GetPlayers()) do
            if p \~= lp and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then   -- Corregido
                ...
                if GetRole(p) == "Murd" and Config.Toggles.Aimbot then
    ...
    RunService.Heartbeat:Connect(function()
        for _, p in pairs(Players:GetPlayers()) do
            ...
            if Config.Toggles.Hitbox and GetRole(p) == "Murd" then
                ...
                if hrp.Size \~= Vector3.new(2, 2, 1) then   -- Corregido
...
local function BuildUI()
    ...
    Notify("CH-HUB V3.2", "Script cargado correctamente", Config.Colors.Accent)
    InitMotors()
    HitboxMaintainer()
end
...
local function RunLogin()
    ...
    f.Position = UDim2.new(0.5, -180, 0.5, -140)
    f.BackgroundColor3 = Config.Colors.Bg
    Instance.new("UICorner", f)
