--CH-HUB MM2 VERSIÓN 3.0 FULL ESTABLE Y OPTIMIZADO XDD
    ...
    connection = RunService.RenderStepped:Connect(function()
        if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") then
            ...
            local color = (role == "Murderer" and Color3.new(1,0,0)) or 
                          (role == "Sheriff" and Color3.new(0,0.7,1)) or 
                          Color3.new(0,1,0)
            
            local enabled = (role == "Murderer" and Config.Toggles.ESP_Murd) or 
...
-- [[ ⚔️ COMBAT ENGINE (AUTO-SHOOT & AIMBOT) ]]
    ...
    RunService.RenderStepped:Connect(function()
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= lp and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if role == "Murderer" and Config.Toggles.Aimbot then
                    -- Aimbot sigue apuntando detrás de paredes
                    camera.CFrame = camera.CFrame:Lerp(CFrame.new(camera.CFrame.Position, hrp.Position), Config.Values.Smooth)
                    
                    -- AUTO-SHOOT (Solo si no hay pared y tienes la Gun)
        ...
        for _, p in pairs(Players:GetPlayers()) do
            ...
            if Config.Toggles.Hitbox and GetRole(p) == "Murderer" then
                ...
                if hrp.Size ~= Vector3.new(2, 2, 1) then
                    hrp.Size = Vector3.new(2, 2, 1); hrp.Transparency = 1; hrp.Color = Color3.new(1, 1, 1); hrp.Material = Enum.Material.Plastic; hrp.CanCollide = true
                end
            end
        end
    end)
...
local function RunLogin()
    ...
    local t = Instance.new("TextLabel", f); t.Size = UDim2.new(1,0,0.3,0); t.Text = "FLOURITE SUPREME V19"; t.TextColor3 = Config.Colors.Accent; t.Font = Enum.Font.GothamBold; t.TextSize = 27; t.BackgroundTransparency = 1
